...
Contrary to what the user interface suggests, the ETS5 does not encrypt your locally stored project files in `C:\ProgramData\KNX\ETS5\ProjectStore` with the project password. Instead, it uses the hard-coded password `ETS5Password` and salt `Ivan Medvedev` to obfuscate specific attributes in the project's XML files. Hard-coded cryptographic secrets are against best practice, as explained in [CWE-321](https://cwe.mitre.org/data/definitions/321.html).
